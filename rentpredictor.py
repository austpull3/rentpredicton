...
if check_password():
    ...
    & (df.type!= 'in-law') & (df.type!= 'land') &(df.type!= 'assisted living')]
        ...
        with tab3:
            ...
            for p in ax.patches:
                ax.annotate('{:.1f}'.format(p.get_height()), (p.get_x()+0.25, p.get_height()+0.01))
            plt.title("Smoking Not Allowed and Allowed", fontsize = 16)
            plt.tick_params(labelsize = 10)
            #st.pyplot(fig)
            plt.xlabel(" ")
            figs2.append(fig)
            ...
            ax3 = sns.boxplot(x = 'laundry_options', y = 'price', data = df, palette = "colorblind")
            sns.set(font_scale = .7)
            plt.title("Laundry Options", fontsize = 16)
            plt.tick_params(labelsize = 10)
            plt.xlabel(" ")
            ...
            fig, ax5 = plt.subplots(figsize = (8, 4.5))
            ax5 = sns.boxplot(x = 'baths', y = 'price', data = df, palette = "colorblind")
            sns.set(font_scale = .7)
            plt.title("Baths", fontsize = 16)
            plt.tick_params(labelsize = 10)
            plt.xlabel(" ")
            figs2.append(fig)
